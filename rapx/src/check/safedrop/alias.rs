...
impl<'tcx> SafeDropGraph<'tcx> {
    ...
    pub fn assign_alias(&mut self, lv_idx: usize, rv_idx: usize) {
        ...
        let r_set_idx = if let Some(idx) = self.alias_graph.find_alias_set(rv_idx) {
            idx
        } else {
            self.alias_graph
                .alias_sets
                .push([rv_idx].into_iter().collect::<FxHashSet<usize>>());
            self.alias_graph.alias_sets.len() - 1
        };

        if let Some(l_set_idx) = self.alias_graph.find_alias_set(lv_idx) {
            if l_set_idx == r_set_idx {
                return;
            }
            self.alias_graph.alias_sets[l_set_idx].remove(&lv_idx);
        }
        self.alias_graph.alias_sets[r_set_idx].insert(lv_idx);

        if self.alias_graph.values[lv_idx].fields.len() > 0
            || self.alias_graph.values[rv_idx].fields.len() > 0
        {
            self.sync_field_alias(lv_idx, rv_idx, 0, true);
        }
        if self.alias_graph.values[rv_idx].father != None {
    ...
    pub fn sync_field_alias(&mut self, lv: usize, rv: usize, depth: usize, clear_left: bool) {
        ...
        if clear_left {
            for lv_field in self.alias_graph.values[lv].fields.clone().into_iter() {
                if let Some(alias_set_idx) = self.alias_graph.find_alias_set(lv_field.1) {
                    self.alias_graph.alias_sets[alias_set_idx].remove(&lv_field.1);
                }
            }
        }
        for rv_field in self.alias_graph.values[rv].fields.clone().into_iter() {
            rap_debug!("rv_field: {:?}", rv_field);
            if !self.alias_graph.values[lv].fields.contains_key(&rv_field.0) {
                let new_index = self.alias_graph.values.len();
                let mut node = Value::new(
                    new_index,
                    self.alias_graph.values[lv].local,
                    self.alias_graph.values[rv_field.1].need_drop,
                    self.alias_graph.values[rv_field.1].may_drop,
                );
                node.kind = self.alias_graph.values[rv_field.1].kind;
            ...
            rap_debug!(
                ...
                self.alias_graph.find_alias_set(rv_field.1)
            );
            if let Some(alias_set_idx) = self.alias_graph.find_alias_set(rv_field.1) {
                self.alias_graph.alias_sets[alias_set_idx].insert(lv_field_value_idx);
            }
            rap_debug!("alias sets: {:?}", self.alias_graph.alias_sets);
    ...
    pub fn sync_father_alias(&mut self, lv: usize, rv: usize, lv_alias_set_idx: usize) {
        ...
        let mut father = self.alias_graph.values[father_id].father.clone();
        while let Some(father_info) = father {
            father_id = father_info.father_value_id;
            let field_id = father_info.field_id;
            let father_value = self.alias_graph.values[father_id].clone();
            if let Some(alias_set_idx) = self.alias_graph.find_alias_set(father_id) {
                for value_idx in self.alias_graph.alias_sets[alias_set_idx].clone() {
                    // create a new node if the node does not exist;
                    let field_value_idx = if self.alias_graph.values[value_idx]
                        .fields
                        .contains_key(&field_id)
                    {
                        *self.alias_graph.values[value_idx]
                            .fields
                            .get(&field_id)
                            .unwrap()
                    } else {
                        ...
                        let mut node = Value::new(
                            new_index,
                            self.alias_graph.values[value_idx].local,
                            self.alias_graph.values[value_idx].need_drop,
                            self.alias_graph.values[value_idx].may_drop,
                        );
                        node.kind = self.alias_graph.values[value_idx].kind;
                        node.father = Some(FatherInfo::new(value_idx, field_id));
                        self.alias_graph.values.push(node.clone());
                        self.alias_graph.values[value_idx]
                            .fields
                    ...
                    self.alias_graph.alias_sets[lv_alias_set_idx].insert(field_value_idx);
                }
            }
    ...
    pub fn projection(&mut self, place: Place<'tcx>) -> usize {
        ...
        for proj in place.projection {
            let new_value_idx = self.alias_graph.values.len();
            match proj {
                ProjectionElem::Deref => {}
                ProjectionElem::Field(field, ty) => {
                    let field_idx = field.as_usize();
                    if !self.alias_graph.values[value_idx]
                        .fields
                        .contains_key(&field_idx)
                    {
                        ...
                        let need_drop = ty.needs_drop(self.alias_graph.tcx(), ty_env);
                        let may_drop = !is_not_drop(self.alias_graph.tcx(), ty);
                        let mut node =
                    ...
                    value_idx = *self.alias_graph.values[value_idx]
                        .fields
                        .get(&field_idx)
                        .unwrap();
                }
    ...
    pub fn handle_fn_alias(&mut self, fn_alias: &MopAliasPair, arg_vec: &Vec<usize>) {
        ...
        let left_local = self.alias_graph.values[lv].local;
        let right_local = self.alias_graph.values[rv].local;

        for index in fn_alias.lhs_fields().iter() {
            if !self.alias_graph.values[lv].fields.contains_key(&index) {
                let new_index = self.alias_graph.values.len();
            ...
            lv = *self.alias_graph.values[lv].fields.get(&index).unwrap();
        }
        for index in fn_alias.rhs_fields().iter() {
            if !self.alias_graph.values[rv].fields.contains_key(&index) {
                let new_index = self.alias_graph.values.len();
            ...
            rv = *self.alias_graph.values[rv].fields.get(&index).unwrap();
        }
    ...
    pub fn merge_alias(&mut self, e1: usize, e2: usize) {
        ...
        let mut s1 = self.alias_graph.find_alias_set(e1);
        let mut s2 = self.alias_graph.find_alias_set(e2);
        ...
        if s1.is_none() {
            ...
            s1 = Some(self.alias_graph.alias_sets.len() - 1);
        }
        ...
        if s2.is_none() {
            ...
            s2 = Some(self.alias_graph.alias_sets.len() - 1);
        }
        ...
        let set2 = self.alias_graph.alias_sets.remove(idx2);
        // If idx2 < idx1, removing idx2 shifts idx1 down by one
        let idx1 = if idx2 < idx1 { idx1 - 1 } else { idx1 };
        self.alias_graph.alias_sets[idx1].extend(set2);

        if self.alias_graph.values[e1].fields.len() > 0 {
            self.sync_field_alias(e2, e1, 0, false);
        }
        if self.alias_graph.values[e2].fields.len() > 0 {
            self.sync_field_alias(e1, e2, 0, false);
        }
        if self.alias_graph.values[e1].father != None {
            self.sync_father_alias(e2, e1, idx1);
        }
        if self.alias_graph.values[e2].father != None {
