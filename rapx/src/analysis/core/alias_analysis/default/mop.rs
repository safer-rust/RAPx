...
impl<'tcx> MopGraph<'tcx> {
    pub(crate) fn compute_path_sensitive_paths(&mut self) -> Vec<Vec<usize>> {
        ...
        self.collect_path_sensitive_paths(
            0,
            &mut current_path,
            &mut active_blocks,
            &mut all_paths,
    ...
    fn collect_path_sensitive_paths(
        &mut self,
        current: usize,
        ...
        let cur_scc = self.cfg_block(current).scc.clone();
        if current == cur_scc.enter && !cur_scc.nodes.is_empty() {
            ...
            for (segment_path, _) in paths_in_scc {
                ...
                let mut nexts: Vec<usize> = self
                    ...
                    .iter()
                    .copied()
                    .filter(|next| self.cfg_block(*next).scc.enter != cur_scc.enter)
                    .collect();
                nexts.sort_unstable();
                nexts.dedup();
        ...
        let mut nexts: Vec<usize> = self.cfg_block(current).next.iter().copied().collect();
        nexts.sort_unstable();
        nexts.dedup();
